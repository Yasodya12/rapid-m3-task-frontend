...
function TaskItem({ task, onEdit, onDelete, onToggleComplete }) {
    return (
        <div className={`flex justify-between items-center p-4 bg-gray-200 rounded ${task.status === 'COMPLETE' ? 'bg-green-100' : ''}`}>
            <div>
                <p className="text-sm text-gray-600">ID: {task.id}</p> {/* Displaying the task ID */}
            ...
            <div className="flex space-x-2">
                <button
                    ...
                    className={`px-2 py-1 text-sm text-white rounded ${task.status === 'COMPLETE' ? 'bg-red-500' : 'bg-blue-500'}`}
                >
